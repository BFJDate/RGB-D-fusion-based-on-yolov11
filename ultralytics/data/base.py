...
# 加入深度图像读取
    def load_image(self, i, rect_mode=True):
        ...
        depth = f.replace('RGB', 'depth')
        depth = depth.replace("jpg","png")
        depth = depth.replace("img","depth")
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                im = cv2.imread(f)  # BGR
                if self.hyp.ch > 3:
                    # im = cv2.merge((cv2.resize(cv2.imread(depth), (1024, 1024), interpolation=cv2.INTER_LINEAR)[..., 0], im))
                    im = cv2.merge((cv2.resize(cv2.imread(depth), (1024, 1024), interpolation=cv2.INTER_LINEAR), im))
            if im is None:
